...
- Printer config: `/home/biqu/printer_data/config/printer.cfg` (UI Machine → Upload albo SSH)
...
- Przygotowano plik `BTT-Pi/mainsail.cfg` zgodny z Klipperem: sekcje `[virtual_sdcard]`, `[pause_resume]`, `[display_status]` oraz makra `PAUSE/RESUME/CANCEL_PRINT`.
- Na urządzeniu plik został skopiowany do `/home/biqu/printer_data/config/mainsail.cfg`.
- `printer.cfg` ma dodane `[include mainsail.cfg]` na samej górze.
...
- Potwierdzono, że konfiguracja zaczęła działać po restarcie Klippera.
...
Jeśli chcesz przenieść konfigurację na inne urządzenie, zacznij od:
1. Skopiowania `BTT-Pi/mainsail.cfg` do `/home/biqu/printer_data/config/mainsail.cfg`
2. Dodania `[include mainsail.cfg]` do `printer.cfg`
3. Restartu Klippera
